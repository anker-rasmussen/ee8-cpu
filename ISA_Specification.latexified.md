...
- `JNZ Rcond address` — Jump if register Rcond $\neq$ 0
...
JNZ R3 loop    ; Jump taken because R3 $\neq$ 0
```
...
**Platform behavior:**
- HALT sets CPU_HALT $\rightarrow$ PC frozen, registers and output port preserved
...
start:  LDI R0 10
        LDI R1 1
loop:   SUB R0 R1 R0
        JNZ R0 loop    ; Jump to 'loop' if R0 $\neq$ 0
        HALT
```
...
14: JNZ R3 2            ; If Gate B selected (R3=1) $\rightarrow$ counting loop
15: LDI R0 5            ; Gate A selected: load count A = 5
                         ; PC wraps 15 $\rightarrow$ 0
                         ; Addr 0: LDI R0 5  (redundant, R0 already 5)
                         ; Addr 1: LDI R2 0x02 (correct: Gate A output)
                         ; Addr 2: OUT R2 $\rightarrow$ counting begins
```
...
|-------|-------------------|----|----|------|-------|
| Setup | 0$\rightarrow$1 | 5 | 0x02 | A | — |
| Batch 1 | 2$\rightarrow$...$\rightarrow$9 (5 items) | 5$\rightarrow$0 | 0x02 | A | 5 |
| Toggle | 10$\rightarrow$11$\rightarrow$12$\rightarrow$13$\rightarrow$14$\rightarrow$2 | 3 | 0x03 | B | — |
| Batch 2 | 2$\rightarrow$...$\rightarrow$9 (3 items) | 3$\rightarrow$0 | 0x03 | B | 3 |
| Toggle | 10$\rightarrow$...$\rightarrow$15$\rightarrow$0$\rightarrow$1$\rightarrow$2 | 5 | 0x02 | A | — |
| Batch 3 | 2$\rightarrow$...$\rightarrow$9 (5 items) | 5$\rightarrow$0 | 0x02 | A | 5 |
| ... | repeats | | | | |

**Pattern:** 5(A), 3(B), 5(A), 3(B), ... $\checkmark$
...
│                     │   XOR Rs1 Rs2 Rd                   │
├─────────────────────┼────────────────────────────────────┤
│ SHIFT               │ COMPARE (result $\rightarrow$ R3)              │
...
│   FAULT     Pause + ALARM                                │
│   SYS_ERR   Illegal insn $\rightarrow$ halt + ALARM (latched)        │
│   CPU_HALT  HALT insn $\rightarrow$ halt, no ALARM (latched)         │
│                                                          │
│   HALTED = STOP | FAULT | SYS_ERR | CPU_HALT             │
│   MOTOR_EN_pin = MOTOR_EN_cpu AND NOT HALTED             │
│   ALARM = FAULT | SYS_ERR                                │
...
```
  ...
  MOV/IN/OUT disambiguation (opcode 0111):
       Rs2=00  MOV Rs1 Rd
       Rs2=01  (undefined $\rightarrow$ SYS_ERR)
       Rs2=10  OUT Rs1
       Rs2=11  IN Rd
...
- **All examples corrected**: fixed register name typos (RO$\rightarrow$R2), fixed polling/counting loop bugs, all examples use new JZ/JNZ Rcond syntax
